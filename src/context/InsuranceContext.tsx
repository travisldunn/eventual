...
// Initial data
export const STRIKE_PRICE = 4371; // Spreadsheet's strike price
...
export const InsuranceProvider: React.FC<{ children: React.ReactNode }> = ({
  children,
}) => {
  const [insuranceData, setInsuranceData] =
    useState<InsuranceData>(initialInsuranceData);
  ...
  const initialPotentialSavings = initialInsuranceData.yearlyData
    ...
    .reduce(
      (sum, y) =>
        sum + (y.withoutLock > STRIKE_PRICE ? y.withoutLock - STRIKE_PRICE : 0),
      0
    );
  const [potentialSavings, setPotentialSavings] = useState<number>(
    initialPotentialSavings
  );
  ...
  const setGrowthRate = (rate: number) => {
    ...
    let prev = baseAmount;
    const yearlyData = [
      { year: 0, withoutLock: baseAmount, withLock: baseAmount },
    ];
    let savings = 0;
    for (let year = 1; year <= 3; year++) {
